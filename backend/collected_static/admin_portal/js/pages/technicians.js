  ...
  render: async function (root, token) {
    ...
    root.addEventListener("click", async (e) => {
      ...
      if (action === "delete") {
        openDrawer(
          "删除技师",
          `<p style="color: #374151; line-height: 1.6;">确定要删除该技师吗？此操作不可恢复。</p>`,
          `<button class="zy-btn zy-btn-secondary zy-btn-sm" id="drawer_cancel">取消</button><button class="zy-btn zy-btn-danger zy-btn-sm" id="drawer_confirm">删除</button>`
        );
        root.querySelector("#drawer_cancel").addEventListener("click", closeDrawer);
